...
{
	const STATUS_PENDING = 167;
	const STATUS_SENDED = 168;
	const STATUS_ERROR = 169;
	/**
	...
	public function rules()
	{
		...
		return array(
			array('destination, headers, title, body, status, date', 'required'),
			array('destination, title', 'length', 'max'=>255),
			array('status', 'length', 'max'=>10),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('id, destination, headers, title, body, status, date', 'safe', 'on'=>'search'),
		);
	}
